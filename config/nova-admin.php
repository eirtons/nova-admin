...
return [
    ...
    */
    'robots_txt' => [
        ...
        // robots 内容含 {url} 占位符，须按运行时域名解析。
        // route_only=false（默认）：落 public/robots.txt 静态文件，Sitemap 行按安装/保存
        //   时的域名生成写入——单域名/每站独立部署适用，由 Web 服务器直接返回静态文件。
        // route_only=true：只存 DB、不落静态文件，强制走 /robots.txt 路由按请求域名动态输出。
        //   站群（一码多域名）须设为 true，否则静态文件会把域名冻结在某次保存时的域名上。
        'route_only'       => env('NOVA_ADMIN_ROBOTS_ROUTE_ONLY', false),
    ],
